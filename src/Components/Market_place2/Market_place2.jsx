...
function Market_place2() {
  ...
  const [Latest_Bids, setLatest_Bids] = useState();
    ...
    try {
      if (text == "sell_and_auction_history_address") {
        ...
        let res = await axios.get(
          `https://server.nftapi.online/sell_and_auction_history_address?useraddress=${useradd}`
        );
        ...
        setIsSpinner(false);
      } else if (useradd == "tranding") {
        setIsSpinner(true);

        let res = await axios.get(
          `https://server.nftapi.online/get_trending_NFTs`
        );
        ...
        hightbider(res?.data?.data[id].itemId);
        setIsSpinner(false);
      } else if (text == "sell_and_auction_history") {
        setIsSpinner(true);

        let res = await axios.get(
          " https://server.nftapi.online/sell_and_auction_history?category=All"
        );
        console.log("UU", res?.data?.data[id].useraddress);
        ...
        let res = await axios.get(
          " https://server.nftapi.online/sell_marketplace_history?category=All"
        );
        ...
        let res = await axios.get(
          " https://server.nftapi.online/OnAuction_marketplace_history?category=All"
        );
        console.log("UU", res?.data?.data[id].useraddress);
        setAll_NFT(res?.data?.data[id]);
        setTimer(res?.data?.data[id].bidEndTime);
  ...
  const hightbider = async (Data) => {
    try {
      if (chainID == 1230) {
        let contractOf = await window.tronWeb
          .contract()
          .at(nftMarketContractAddress);
        ...
        let hightbid = await nftContractOf.methods
          .highestBidderMapping(Data)
          .call();
      ...
      if (SendAddress.toUpperCase() !== metaAddress.toUpperCase()) {
        if (base_price <= getInputdata) {
          ...
          let contractOf = await window.tronWeb
            ...
            .at(nftMarketContractAddress);
          await contractOf
            .createBidOnItem(itemId, nftcontactadd)
            .send({
              ...
              if (hash != "") {
                try {
                  let postapi = await axios.post(
                    "https://server.nftapi.online/trending_NFTs",
                    {
                  ...
                  let res = await axios.get(
                    `https://server.nftapi.online/get_user_profile?address=${metaAddress.toUpperCase()}`
                  );
                  ...
                  if (res.data.success) {
                    let History = await axios.post(
                      "http://localhost:3344/create_Bid_History",
                      {
                        address: metaAddress,
                        username: res.data.data.username,
                        price: `${getInputdata} Tron`,
                        image: All_NFT.url,
                        edate: new Date(),
                      }
                    );
                    let create_Latest_Bids = await axios.post(
                      "http://localhost:3344/create_Latest_Bids",
                      {
                        address: metaAddress,
                        username: res.data.data.username,
                        price: `${getInputdata} Tron`,
                    ...
                    console.log("ResPostApi", create_Latest_Bids);
                  } else {
                  }
          ...
          toast.error(
            "Bid price must be greater than base price and highest bid"
          );
          setIsSpinner(false);
        }
      } else {
          ...
          try {
            ...
            if (SendAddress.toUpperCase() !== acc.toUpperCase()) {
              ...
              if (base_price <= getInputdata) {
                ...
                let res = await axios.get(
                  `https://server.nftapi.online/get_user_profile?address=${acc.toUpperCase()}`
                );
      ...
      if (metaAddress == null) {
        ...
        if (HighestBideradd.toUpperCase() == metaAddress.toUpperCase()) {
          ...
          let postapiPushdata = await axios.post(
            "https://server.nftapi.online/update_auction_status",
            {
              tokenid: tokenId,
            }
          );

          let udate_Tranding = await axios.post(
            "https://server.nftapi.online/update_tranding",
            {
              tokenId: tokenId,
            }
          );
      ...
      if (metaAddress == null) {
        ...
        try {
          if (HighestBideradd.toUpperCase() == metaAddress.toUpperCase()) {
            ...
            await nftContractOf.methods
              .claimBidItem(itemId, nftcontactadd)
              .send({
                from: acc,
                value: 0,
              });
  ...
  const purchaseOrder = async () => {
    if (chainID == 1230) {
      ...
      if (SendAddress.toUpperCase() === metaAddress.toUpperCase()) {
        ...
        await contractOf
          .createMarketSale(itemId, nftcontactadd)
          .send({
            callValue: inputdata,
          })
          .then(async (hash) => {
            if (hash != "") {
              try {
                let postapiPushdata = await axios.post(
                  "https://server.nftapi.online/update_sell_status",
                  {
                    tokenid: tokenId,
                  }
                );
  ...
  const get_Bid_History_Data = async () => {
    try {
      let res = await axios.get("http://localhost:3344/get_Bid_History");
      setBidHistory(res.data.data);

      let get_Latest_Bids = await axios.get(
        `http://localhost:3344/get_Latest_Bids?nftName=${All_NFT.name}`
      );
      // console.log("get_Latest_Bids",get_Latest_Bids);
      setLatest_Bids(get_Latest_Bids.data.data);
    } catch (e) {
      console.log("Error While Get Bid History Data", e);
    }
  };

  useEffect(() => {
    get_Bid_History_Data();
  });
    ...
    <div>
        ...
        <div class="container">
          <div class="inner-title">
            ...
            <ul>
              ...
              <li>
                <Link className="text-white" to="/market_place">
                  MarketPlace
                </Link>
              </li>
              <li>
            ...
            <div class="inner-shape">
              <img
                src="https://gible-nft.hibootstrap.com/images/inner-banner/inner-shape1.png"
                alt="Images"
              />
              <img
                src="https://gible-nft.hibootstrap.com/images/inner-banner/inner-shape2.png"
                alt="Images"
              />
            </div>
          </div>
      ...
      <div className="container-fluid mt-3">
              ...
              <ul className="mt-4">
                <li>
                  ...
                  <b>
                    {" "}
                    &nbsp;&nbsp;{" "}
                    <span className="fs-5 text_color">{All_NFT.name}</span>
                  </b>
                </li>
                <li>
                  <h5>Current Price :</h5>
                  <b>
                    ...
                    <span className="fs-5 text_color">
                      {All_NFT.price}{" "}
                      {chainID == 1230 ? "TRX" : chainID == 1 ? "ETH" : "BNB"}
                    </span>
                  </b>
                </li>
            ...
            {index == 1 ? (
              <div class="item-details-in-content">
                <div class="item-left">
                  ...
                  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                  <Countdown
                    date={Date.now() + (parseInt(Timer) * 1000 - Date.now())}
                    renderer={renderer}
                  />
                </div>
                <div class="item-right">
                  <h3 class="item-remaining">Highest Bid</h3>
                  <h3 class="item-right-eth">
                    {hightbid}{" "}
                    {chainID == 1230 ? "TRX" : chainID == 1 ? "ETH" : "BNB"}{" "}
                  </h3>
                </div>
                    ...
                    <>
                      <div class="preview-box">
                        ...
                        <input
                          ...
                          placeholder={
                            chainID == 1230
                              ? "Enter Bid Value in TRX"
                              : chainID == 1
                              ? "Enter Bid Value in ETH"
                              : "Enter Bid Value in BNB"
                          }
                          onChange={(e) => setgetInputdata(e.target.value)}
                        />
                      </div>
                      <div
                        class="item-details-btn mt-4"
                    ...
                    <div className="col text-end downtext">
                      <p>
                        ...
                        <a
                          href={
                            chainID == 1230
                              ? `https://tronscan.org/#/contract/${All_NFT.nftContract}`
                              : chainID == 56
                              ? `https://bscscan.com/address/${All_NFT.nftContract}`
                              : `https://etherscan.io/address/${All_NFT.nftContract}`
                          }
                          target="_blank"
                        >
                          {" "}
                          {All_NFT.nftContract?.substring(0, 8) +
                            "..." +
                            All_NFT.nftContract?.substring(
                              All_NFT.nftContract?.length - 8
                            )}
                        </a>{" "}
                      </p>
                      <p>{All_NFT.tokenId}</p>
                      ...
                      <p>
                        ...
                        <a
                          ...
                          target="_blank"
                        >
                          {All_NFT.useraddress?.substring(0, 8) +
                            "..." +
                            All_NFT.useraddress?.substring(
                              All_NFT.useraddress?.length - 8
                            )}
                        </a>
                      </p>
                    </div>
        ...
        {index == 1 ? <>
          ...
          <div className="col-lg-6">
            <div class="item-details-content">
              ...
              <div class="item-details-into">
                <div class="row">
                  {BidHistory?.map((items, index) => {
                    let Times = Math.floor(
                      new Date(items.edate).getTime() / 1000.0
                    );
                    let currentTime = Math.floor(new Date().getTime() / 1000.0);

                    let FinalTime = Number(currentTime) - Number(Times);
                    let days = parseInt(FinalTime / 86400);
                    FinalTime = FinalTime % 86400;
                    let hours = parseInt(FinalTime / 3600);

                    FinalTime %= 3600;
                    let munites = parseInt(FinalTime / 60);

                    FinalTime %= 60;
                    let second_here = parseInt(FinalTime);
                        ...
                        <div class="col-lg-12">
                          <div class="item-details-card">
                            <div class="item-details-card-img">
                              <img src={items?.image} alt="Images" height="50rem" width="50rem" />
                              <i class="fa-solid fa-check"></i>

                            </div>
                            <div class="item-details-card-content">
                              <h3>
                                Bid Placed For <b>{items?.price}</b>
                              </h3>
          ...
          <div className="col-lg-6">
            <div class="item-details-user-area">
              <h3>Latest Bids </h3>
              <div class="item-details-user-into">
                <div class="row">
                  {Latest_Bids?.map((items, index) => {
                    let EpochTimer = Math.floor(
                      new Date(items.edate).getTime() / 1000.0
                    );
                    let bidingTime = new Date(EpochTimer * 1000);
                      ...
                      <>
                        <div class="col-lg-12">
                          <div class="item-details-user-card">
                            <div class="item-details-card-img">
                              <img
                                src={`https://server.nftapi.online/uploads/${items.image}`}
                                alt="Images"
                              />
                            </div>
                            <div class="item-details-card-content">
                              <h3>
                                By <b>{items.username}</b>
                              </h3>
                              <span>
                                Bid <b> At {items.price} </b>
                              </span>
                            </div>
                            <div class="item-details-card-right">
                              <h3>{items.price}</h3>
                              <span>{bidingTime.toLocaleString()}</span>
                            </div>
                          </div>
                        </div>
                      </>
